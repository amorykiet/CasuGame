...
char **GuiLoadIcons(const char *fileName, bool loadIconsName)
{
    ...
    if (rgiFile != NULL)
    {
            ...
            (signature[3] == ' '))
        {
            ...
            size_t totalElements = iconCount * (iconSize * iconSize / 32);
            if (totalElements > SIZE_MAX / sizeof(unsigned int)) {
                // Handle error: size is too large
            } else {
                fread(guiIconsPtr, sizeof(unsigned int), totalElements, rgiFile);
            }
        }
